...
def compute_stats(logdir):
    ...
    exp_stats = {
        ...
        "total_successful_human_actions": 0,
        "total_idle_time": 0,
        "total_reward": 0,
        ...
        "cumulative_successful_human_actions": [0],
        "cumulative_total_human_actions": [0],
    ...
    for t in range(len(raw_data)):
        num_switches = np.sum(
            np.abs(assignments - raw_data[t]["assignments"]).sum(1) > 0
        )
        exp_stats["total_switches"] += num_switches
        exp_stats["cumulative_switches"].append(
            exp_stats["cumulative_switches"][-1] + num_switches
        )
        ...
        exp_stats["cumulative_hard_resets"].append(
            exp_stats["cumulative_hard_resets"][-1]
        )
        exp_stats["cumulative_human_actions"].append(
            exp_stats["cumulative_human_actions"][-1]
        )
        ...
        exp_stats["total_human_actions"] = raw_data[t]["total_human"]
        exp_stats["cumulative_total_human_actions"].append(
            exp_stats["cumulative_total_human_actions"][-1] + raw_data[t]["total_human"]
        )

        for i in range(exp_cfg.num_envs):
            exp_stats[i]["cumulative_hard_resets"].append(
                exp_stats[i]["cumulative_hard_resets"][-1]
            )
            exp_stats[i]["cumulative_human_actions"].append(
                exp_stats[i]["cumulative_human_actions"][-1]
            )
            exp_stats[i]["cumulative_idle_time"].append(
                exp_stats[i]["cumulative_idle_time"][-1]
            )
            exp_stats[i]["cumulative_viols"].append(
                exp_stats[i]["cumulative_viols"][-1]
            )
            exp_stats[i]["cumulative_successes"].append(
                exp_stats[i]["cumulative_successes"][-1]
            )
            ...
            if use_human_ac:
                exp_stats["total_successful_human_actions"] += 1
                exp_stats["cumulative_successful_human_actions"][-1] += 1
        ...
        exp_stats["ROHE"].append(
            exp_stats["cumulative_successes"][-1]
            / (exp_stats["cumulative_total_human_actions"][-1] + 1)
        )
    ...
    return (
        ...
        exp_stats["total_successes"],
        exp_stats["total_viols"],
        exp_stats["total_switches"],
        exp_stats["total_human_actions"],
        exp_stats["total_successful_human_actions"],
        exp_stats["total_idle_time"],
        exp_stats["ROHE"][-1],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "logdir", type=str, help="filepath to log directory (parent of raw_data.pkl)"
    )
