...
class ParallelExperiment:
    def __init__(self, exp_cfg, agent, supervisor, allocation, network):
        ...
        pickle.dump(self.exp_cfg, open(os.path.join(self.logdir, "args.pkl"), "wb"))
    ...
    def experiment_setup(self, supervisor):
        ...
        if self.vec_env:
            ...
            envs = [
                make_env(self.exp_cfg.env_name, idx=i)
    ...
    def assign_humans(self, env_priorities):
        ...
        while human_idx < self.exp_cfg.num_humans:
            ...
            # If we have a human we can re-assign and a robot that doesn't already have a human, re-assign them
                ...
                # If the connection fails, don't assign the human
        ...
        self.total_humans = np.sum(self.assignments) + failed_assignments
        """
        # Find number of humans that could possibly be reassigned
        max_humans_to_assign = 0
        for i in range(self.exp_cfg.num_humans):
    ...
    def assign_humans_sequentially(self, allocation_metrics, network):
        ...
        while human_idx < self.exp_cfg.num_humans:
            ...
            # free humans if possible
                ...
                self.exp_cfg.num_envs, self.exp_cfg.num_humans
            ):
                if self.assignments[:, human_idx].sum():
                    current_env = np.argmax(self.assignments[:, human_idx])
                    self.assignments[current_env][human_idx] = 0
        ...
        # Total Attempted Allocations are the number of failed allocations and successfullse
        self.total_humans = np.sum(self.assignments) + len(failed_allocations)
        """
        # Find number of humans that could possibly be reassigned
        max_humans_to_assign = 0
        for i in range(self.exp_cfg.num_humans):
    ...
    def vec_step(self, action_list, allocation_metrics):
        ...
        if self.exp_cfg.allocation == "ASM":
            ...
            # Get env priorities
            env_priorities = self.allocation.allocate(allocation_metrics, self.network)
        ...
        for env_idx in range(self.exp_cfg.num_envs):
            ...
            assert (
                self.exp_cfg.num_humans == 0 or 0 <= self.assignments.sum(0).max() <= 1
            )
            ...
            if use_human_ac:
                ...
                # Otherwise get human action and override policy action
                    ...
                    human_action = self.supervisor.get_action(
                        self.state[env_idx], env_idx
                    )
        ...
        if self.exp_cfg.noise:
            actions += np.random.normal(
                scale=self.exp_cfg.noise, size=actions.shape
            ).astype(
                actions.dtype
            )  # noise injection
        next_states, rewards, dones, _ = self.envs.step(
            torch.tensor(actions, device=self.envs.rl_device)
        )
        ...
        for env_idx in range(self.exp_cfg.num_envs):
            if ret[env_idx] == 1:
                ...
                ret[env_idx] = (
                    np.copy(self.state[env_idx].cpu().numpy()),
                    actions[env_idx],
                    rewards[env_idx].cpu().numpy(),
                    next_states[env_idx].cpu().numpy(),
                    dones[env_idx].cpu().numpy(),
                    info,
                )
    ...
    def step(self, action_list, allocation_metrics):
        ...
        for env_idx in range(self.exp_cfg.num_envs):
            ...
            if action == "reset":
                ...
                if self.exp_cfg.noise:
                    action += np.random.normal(
                        scale=self.exp_cfg.noise, size=action.shape
                    )
            ...
            # if we hit a successful sink state, we are allowed a free reset or time out
            if (
                done or self.episode_steps[env_idx] >= env.max_episode_steps
            ) and not env.constraint:
    ...
    def reset_all(self):
        ...
        ]  # number of timesteps remaining for hard resets to complete
        self.t = 0
        self.reset_time_idx = -1
        if self.vec_env:
            self.envs.reset_idx(
                torch.arange(self.exp_cfg.num_envs, device=self.envs.device)
            )
            self.envs.reset()
            self.state = self.envs.obs_buf.clone()
        else:
    ...
    def get_demos(self, suffix="task", suffix2=""):
        ...
        if self.vec_env:
            p = pickle.load(
                open(
                    "env/assets/isaacgym/demos/{}/{}{}.pkl".format(
                        suffix, self.exp_cfg.env_name, suffix2
                    ),
                    "rb",
                )
            )
            return [
                (p["obs"][i], p["act"][i], p["rew"][i], p["obs2"][i], 1 - p["done"][i])
                for i in range(p["obs"].shape[0])
            ]
        else:
            constraint = suffix == "constraint"
            return self.envs[0].get_offline_data(
                (
                    self.exp_cfg.agent_cfg.num_unsafe_transitions
                    if constraint
                    else self.exp_cfg.agent_cfg.num_task_transitions
                ),
    ...
    def run(self):
        self.reset_all()
        if (
            "pretrain_qrisk" in self.exp_cfg.agent_cfg
            and self.exp_cfg.agent_cfg.pretrain_qrisk
        ):
            if (
                "safety_critic" in self.exp_cfg.agent_cfg
                and self.exp_cfg.agent_cfg.safety_critic
            ):
                constraint_demo_data = self.get_demos(suffix="constraint")
                self.agent.pretrain_critic_safe(constraint_demo_data)
            if (
                "goal_critic" in self.exp_cfg.agent_cfg
                and self.exp_cfg.agent_cfg.goal_critic
            ):
        ...
        if "task_demos" in self.exp_cfg.agent_cfg and self.exp_cfg.agent_cfg.task_demos:
            task_demo_data = self.get_demos(
                suffix="task", suffix2="BC" if self.exp_cfg.agent_cfg.bc else ""
            )
        ...
        while self.t < self.exp_cfg.num_steps:
            ...
            if self.t % self.exp_cfg.log_freq == 0:
                succ, rohe = self.dump_logs()
                self.agent.save()
                print("LOGDIR: ", self.logdir)
            self.t += 1
        return succ, rohe * 100
