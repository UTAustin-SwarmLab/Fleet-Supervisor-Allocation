...
def torchify(x, device):
    return (
        x.to(device) if isinstance(x, torch.Tensor) else torch.FloatTensor(x).to(device)
    )
...
class SingleTaskParallelILAgent(ParallelAgent):
    def __init__(self, envs, exp_cfg, logdir):
        ...
        self.ensemble_memories = [
            ReplayMemory(self.cfg.replay_size, exp_cfg.seed + i)
    ...
    def agent_setup(self):
        ...
        agent = BC(obs_space, act_space, self.exp_cfg, self.logdir)
        return agent
    ...
    def pretrain_critic_safe(self, constraint_demo_data):
        ...
        for transition in constraint_demo_data:
            ...
            if self.num_unsafe_transitions == self.cfg.num_unsafe_transitions:
                break
        print("Number of Constraint Transitions: ", self.num_unsafe_transitions)
        print("Number of Constraint Violations: ", self.num_constraint_violations)
        batch_size = self.cfg.batch_size
        if self.cfg.pos_fraction > 0:
            batch_size = min(
                self.cfg.batch_size,
                int(self.num_constraint_violations / self.cfg.pos_fraction),
            )
        for i in range(self.cfg.critic_safe_pretraining_steps):
            ...
            self.forward_agent.safety_critic.update_parameters(
                ...
                batch_size=min(batch_size, len(constraint_demo_data)),
            )
    ...
    def pretrain_critic_goal(self, data):
        ...
        if self.cfg.pos_fraction > 0:
            batch_size = min(
                self.cfg.batch_size, int(self.num_goal_reached / self.cfg.pos_fraction)
            )
        for i in range(self.cfg.critic_safe_pretraining_steps):
            if i % 100 == 0:
    ...
    def pretrain_with_task_data(self, task_demo_data):
        ...
        for i in range(self.cfg.num_policies):
            for _ in range(self.human_memory.size):
                elem = self.human_memory.buffer[
                    np.random.randint(self.human_memory.size)
                ]
                self.ensemble_memories[i].push(
                    elem[0].copy(), elem[1].copy(), elem[2], elem[3].copy(), elem[4]
                )
    ...
    def add_transitions(self, transitions):
        ...
        for t in transitions:
            if t is not None:
                ...
                if info["human"]:
                    add_transition(
                        self.human_memory, state, action, reward, next_state, mask
                    )
                    for i in range(self.cfg.num_policies):
                        elem = self.human_memory.buffer[
                            np.random.randint(self.human_memory.size)
    ...
    def train(self, t):
        if len(self.human_memory) > self.cfg.batch_size:
            ...
            for i in range(self.cfg.updates_per_step):
                if len(self.human_memory) > self.cfg.batch_size:
                    if self.exp_cfg.discrete:
                        self.forward_agent.train_discrete(
                            memory=self.human_memory, batch_size=self.cfg.batch_size
                        )
                    else:
                ...
                if (
                    self.cfg.safety_critic
                    and not self.cfg.disable_online_updates
                    and len(self.recovery_memory) > self.cfg.batch_size
                    and self.num_constraint_violations / self.cfg.batch_size
                    > self.cfg.pos_fraction
                ):
                    self.forward_agent.safety_critic.update_parameters(
                        memory=self.recovery_memory,
                        agent=self.forward_agent,
                ...
                if (
                    self.cfg.goal_critic
                    and not self.cfg.disable_online_updates
                    and len(self.goal_memory) > self.cfg.batch_size
                    and self.num_goal_reached / self.cfg.batch_size
                    > self.cfg.pos_fraction
                ):
                    self.forward_agent.goal_critic.update_parameters(
                        memory=self.goal_memory,
    ...
    def get_allocation_metrics(self, states, t):
        ...
        if self.cfg.goal_critic:
            td_prob = (
                self.forward_agent.goal_critic.get_value(
                    torchify(states, self.device), torchify(actions, self.device)
                )
                .cpu()
                .numpy()
            )
        ...
        if self.cfg.safety_critic:
            risk = (
                self.forward_agent.safety_critic.get_value(
                    torchify(states, self.device), torchify(actions, self.device)
                )
                .cpu()
                .numpy()
