...
class BC(object):
    def __init__(self, observation_space, action_space, args, logdir):
        ...
        self.safety_critic = QRiskWrapper(
            observation_space,
            action_space,
    ...
    def init_policies(self):
        ...
        if args.discrete:
            ...
            self.policy_optims = [
                Adam(self.policy[i].parameters(), lr=args.agent_cfg.lr)
    ...
    def save(self):
        ...
        torch.save(
            self.safety_critic.safety_critic.state_dict(),
            osp.join(self.logdir, "safety_critic.ckpt"),
        )
    ...
    def load(self, logdir):
        ...
        #     policy.eval()
        self.safety_critic.safety_critic.load_state_dict(
            torch.load(osp.join(logdir, "safety_critic.ckpt"))
        )
    ...
    def get_policy_uncertainty(self, states):
        ...
        if self.discrete:
            ...
            max_variance = np.square(
                np.std(
                    np.array([self.action_space.low, self.action_space.high]), axis=0
                )
            ).mean()
            return (
                np.square(np.std(np.array(outputs), axis=0)).mean(axis=1) / max_variance
            )

    def get_cosine_similarity(self, states):
        """
        Get cosine similarity between different robots with given states as input and return nxn matrix with
        pairwise cosine similarity between robots
        """
    ...
    def train(self, memory, batch_size):
        ...
        for i, policy in enumerate(self.policy):
            state_batch, action_batch, _, _, _ = memory[i].sample(batch_size=batch_size)
    ...
    def retrain(self, memory, batch_size):
        """
