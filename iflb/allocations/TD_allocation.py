...
class TDAllocation(Allocation):
    ...
    def allocate(self, allocation_metrics, network):
        ...
        risk_mean, risk_std = np.mean(np.array(self.risk_data).squeeze()), np.std(
            np.array(self.risk_data)
        )
        novelty_mean, novelty_std = np.mean(np.array(self.novelty_data)), np.std(
            np.array(self.novelty_data)
        )
