...
class RandomAllocation(Allocation):
    ...
    def allocate(self, allocation_metrics, network):
        ...
        # gaussian noise
        busy_humans = round(
            np.random.normal(mean_busy_humans, scale=(self.cfg.std_dev * num_humans))
        )
        ...
        for i in range(num_humans):
            if (
                human_timers[i] > 0 and human_timers[i] < self.exp_cfg.min_int_time
            ) or (
                np.max(assignments[:, i]) > 0
                and blocked_envs[np.argmax(assignments[:, i])]
            ):
