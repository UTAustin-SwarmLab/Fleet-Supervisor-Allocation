...
class NCURAllocation(Allocation):
    """
    ...
    def allocate(self, allocation_metrics, network):
        if "C" in self.cfg.order:
            ...
            ), "Agent {} does not provide the required metrics for {} allocation.".format(
                self.exp_cfg.agent, self.cfg.order
            )
        if "R" in self.cfg.order:
            assert (
                "risk" in allocation_metrics
            ), "Agent {} does not provide the required metrics for {} allocation.".format(
                self.exp_cfg.agent, self.cfg.order
            )
        if "U" in self.cfg.order:
            assert (
                "uncertainty" in allocation_metrics
            ), "Agent {} does not provide the required metrics for {} allocation.".format(
                self.exp_cfg.agent, self.cfg.order
            )
        ...
        for i in range(self.exp_cfg.num_envs):
            ...
            if constraint_violation:
                ...
                if self.cfg.warmup_penalty > allocation_metrics["time"]:
                    cv = (
                        -1
                    )  # penalize instead of prioritize constraint violation during warmup
